...
class NotiTableViewController: UITableViewController {
    let notiManager = MyNotiManager.getInstance()
    ...
    func requestAuth(){
        if #available(iOS 10.0, *){
            UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge], completionHandler: { granted, error in
                if granted {
                    print("Agree notification")
                }
                else {
                    print("Disagree notification")
                }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Configure the cell...
        cell.textLabel?.text = notiManager.getItem(row: indexPath.row).event
        return cell
    }
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            notiManager.delete(row: indexPath.row)
